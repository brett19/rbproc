...
ModelList.load = function(path, callback) {
  RoseLoader.load(path, function(err, rh) {
    ...
    for (i = 0; i < meshes; ++i) {
      data.meshes.push(rh.readNtString());
    }

    materials = rh.readUint16();
    for (i = 0; i < materials; ++i) {
      var material = new ModelList.Material();
      material.texturePath       = rh.readNtString();
      ...
      material.glowColour        = rh.readColour3();
      ...
      if (parts > 0) {
          ...
          while ((type = rh.readUint8()) != 0) {
            ...
            switch (type) {
              case ModelList.PROPERTY_TYPE.POSITION:
                part.position = rh.readVector3().multiplyScalar(0.01);
                break;
              ...
              case ModelList.PROPERTY_TYPE.CONSTANT_ANIMATION:
                part.animPath = rh.readString(size);
                break;
              ...
              case ModelList.PROPERTY_TYPE.USE_LIGHTMAP:
                part.useLightmap = rh.readUint16() !== 0;
                break;
        ...
        for (j = 0; j < effects; ++j) {
          ...
          while ((type = rh.readUint8()) != 0) {
            ...
            switch (type) {
              case ModelList.PROPERTY_TYPE.POSITION:
                effect.position = rh.readVector3().multiplyScalar(0.01);
                break;
              case ModelList.PROPERTY_TYPE.ROTATION:
